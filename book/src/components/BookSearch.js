    ...
    const onSearch = async() => {
        if (library) {
            axios.get(
                ...
                if (response.data.response.header.resultCode === "S004") {
                    alert("검색 결과가 없습니다!");
                }
            ...
            {library === '' ? (
                ...
                <div className="overflow-x-auto">
                    <label className="input input-bordered flex items-center gap-2" style={{width:'500px', margin:'0 auto'}}>
                        <input type="text" className="grow" placeholder="검색어를 입력하세요" onChange={keyword} onKeyDown={enterkey}/>
                        <button onClick={onSearch}>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4 opacity-70"><path fillRule="evenodd" d="M9.965 11.026a5 5 0 1 1 1.06-1.06l2.755 2.754a.75.75 0 1 1-1.06 1.06l-2.755-2.754ZM10.5 7a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0Z" clipRule="evenodd" /></svg>
                        </button>
                    </label>
                    <p>* 최대 50개의 검색결과만 출력됩니다.</p>
